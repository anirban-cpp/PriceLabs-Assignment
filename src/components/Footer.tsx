    ...
    return (
        <footer className='w-full p-5 bg-white flex justify-between'>
            ...
            <div className='flex-1 gap-6 flex justify-end items-center'>
                <button onClick={handlePreviousPage} className={`${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}  rounded-full p-2 border border-gray-400`} disabled={currentPage === 1}>
                ...
                <button onClick={handleNextPage} className={`${currentPage === pageInfo?.totalPages ? 'opacity-50 cursor-not-allowed' : 'opacity-100'} rounded-full p-2 border border-gray-400`} disabled={currentPage === pageInfo?.totalPages}>
